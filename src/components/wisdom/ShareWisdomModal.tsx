...
import { X, Search, Users, Trash2, UserPlus, Loader2, CheckCircle, AlertCircle } from 'lucide-react'
import { createPortal } from 'react-dom'

interface ShareWisdomModalProps {
  isOpen: boolean
  onClose: () => void
  wisdomId: string
  wisdomTitle?: string
...
export default function ShareWisdomModal({ isOpen, onClose, wisdomId, wisdomTitle }: ShareWisdomModalProps) {
  ...
  const loadShares = async () => {
    ...
    try {
      const res = await fetch(`/api/wisdom/${wisdomId}/share`)
      if (res.ok) {
  ...
  const handleAddContact = useCallback(async (contact: Contact) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || 'Failed to share wisdom. Please try again.')
        setAddingUser(null)
        return
      }
  ...
  const handleRemoveShare = useCallback(async (contactId: string) => {
    try {
      const res = await fetch(`/api/wisdom/${wisdomId}/share?contact_id=${contactId}`, {
        method: 'DELETE',
      })
      if (res.ok) {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-200">
          <div>
            <h3 className="text-lg font-semibold text-[#2d2d2d]">Share Wisdom</h3>
            {wisdomTitle && (
              <p className="text-sm text-gray-500 mt-0.5 truncate max-w-[280px]">{wisdomTitle}</p>
            )}
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
                ...
                {searchResults.map(contact => (
                  ...
                  >
                    ...
                    {contact.relationship_type && (
                      <span className="px-2 py-1 bg-[#4A3552]/10 text-[#4A3552] text-xs rounded-full">
                        {contact.relationship_type}
                      </span>
                    )}
                    {addingUser === contact.id ? (
                      <Loader2 size={18} className="text-[#4A3552] animate-spin" />
                    ) : (
                      <UserPlus size={18} className="text-[#4A3552]" />
                    )}
                  </button>
                ))}
              </div>
          ...
          {/* Info note */}
          <div className="p-4 bg-[#4A3552]/5 rounded-xl border border-[#4A3552]/10">
            <p className="text-sm text-[#4A3552]">
              <strong>Sharing wisdom</strong> allows contacts to view this entry and leave comments.
              They&apos;ll receive a notification to view and engage with your wisdom.
            </p>
          </div>
          ...
          {loadingShares ? (
            ...
            <div className="text-center py-6 text-gray-500">
              ...
              <p className="text-xs">Search your contacts above to share this wisdom</p>
            </div>
          )}
        </div>
        ...
        <div className="p-5 border-t border-gray-200 flex justify-end">
          <button
            onClick={onClose}
            className="px-6 py-2.5 bg-[#4A3552] hover:bg-[#5a4562] text-white rounded-xl font-medium transition-colors"
          >
            Done
          </button>
        </div>
