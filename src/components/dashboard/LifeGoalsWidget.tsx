        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {SUGGESTIONS.filter(s => !goals.includes(s)).map((s) => (
              <button key={s} onClick={() => addGoal(s)} className="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white text-sm rounded-full transition-colors">{s}</button>
            ))}
          </div>
        </div>
      </Modal>
