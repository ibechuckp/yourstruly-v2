  ...
  return (
    <div className="min-h-screen p-6">
      ...
      <div className="flex items-center gap-4 mb-8">
        <Link 
          href="/dashboard" 
          className="p-2 bg-black/40 rounded-xl text-white/70 hover:bg-white/20 hover:text-white transition-all"
        >
          <ChevronLeft size={20} />
        </Link>
        <div>
      ...
      <section className="mb-10">
        ...
        {contacts.length === 0 ? (
          <div className="bg-black/40 rounded-2xl p-8 border border-white/20 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {contacts.map(contact => (
              <div 
                key={contact.id} 
                className="bg-black/40 rounded-2xl p-5 border border-white/20 hover:bg-white/15 hover:border-amber-500/30 transition-all group"
              >
        ...
        {pets.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {pets.map(pet => (
              <div 
                key={pet.id} 
                className={`bg-black/40 rounded-2xl p-5 border border-white/20 hover:bg-white/15 transition-all group ${pet.is_deceased ? 'opacity-70' : 'hover:border-pink-500/30'}`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
