...
| Type | Icon | Purpose |
|------|------|---------|
| photo_backstory | 📸 | Get story behind a photo |
| tag_person | 👤 | Identify person in photo |
| missing_info | 📝 | Fill contact's missing field |
| quick_question | 👤 | Quick contact verification |
| memory_prompt | 💭 | Record a memory |
| knowledge | 🧠 | Capture wisdom/advice |
| postscript | 💌 | Create future message |
| favorites_firsts | 🏆 | Document favorites/firsts |

### Key Files
- `src/app/(dashboard)/dashboard/page.tsx` — Main dashboard with inline bubble rendering
- `src/components/engagement/EngagementBubbles.tsx` — Standalone component (not used by dashboard)
- `src/components/engagement/Bubble.tsx` — Individual bubble component
- `src/hooks/useEngagementPrompts.ts` — Data fetching hook
- `src/styles/engagement.css` — Bubble styles
- `src/styles/home.css` — Dashboard/home page styles

### UI Specs (as of 2026-02-21)
- **Tile size:** 240x210px
- **Gap:** 24px
- **Container:** 816x468px (3 cols × 2 rows)
...
- `birth_date` / `date_of_birth` → Date picker
- `phone` → Tel input
- `email` → Email input
- `contact_info` → Phone + Email fields
- Other → Textarea

## Views (Database)
- `contacts_missing_info` — Contacts with null birth_date, how_met, or contact info
...
## Git History (Recent)
- `db2f82f` — Add progress tracker + fix contact names + UI improvements
- `3efeb8e` — UI improvements: larger tiles, better spacing, contact forms
...
**Issues Fixed:**
1. Tiles overlapping/moving with cursor → Fixed layout, removed rotation/hover effects
2. "Unknown Contact" showing → Added fallback to user's contacts list
3. Tile transparency → Changed to solid white background
4. Tiles draggable → Disabled with draggable={false}
5. Counter above tiles → Replaced with progress tracker
6. No progress visualization → Added animated progress tracker bar
